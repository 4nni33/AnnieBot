...
*⋆ ࣪.𓏲 ๋࣭ Puedes contactar a la propietaria del Bot si crees que se trata de un error (TENER PRUEBAS) para tratar el motivo de tú desbaneo*
⋆ ࣪.𓏲 ๋࣭ wa.me/59892959423
`.trim();
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    await m.reply(`*[ ⚠️ 𝚁𝙴𝙿𝙾𝚁𝚃𝙴 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙲𝙾𝙽 𝙵𝙰𝙻𝙻𝙾𝚂 ⚠️ ]*\n\n*⋆ ࣪.𓏲 ๋࣭ 𝙿𝙻𝚄𝙶𝙸𝙽:* ${m.plugin}\n*⋆ ࣪.𓏲 ๋࣭ 𝚄𝚂𝚄𝙰𝚁𝙸𝙾:* ${m.sender}\n*⋆ ࣪.𓏲 ๋࣭ 𝙲𝙾𝙼𝙰𝙽𝙳𝙾:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n*[❗] 𝚁𝙴𝙿𝙾𝚁𝚃𝙴𝙻𝙾 𝙰𝙻 𝙲𝚁𝙴𝙰𝙳𝙾𝚁 𝙳𝙴𝙻 𝙱𝙾𝚃 𝙿𝙰𝚁𝙰 𝙳𝙰𝚁𝙻𝙴 𝚄𝙽𝙰 𝚂𝙾𝙻𝚄𝙲𝙸𝙾𝙽, 𝙿𝚄𝙴𝙳𝙴 𝚄𝚂𝙰𝚁 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 #reporte*`.trim(), data.jid)
                            }
                        await m.reply(text)
                    }
                } finally {
...
╭┈┈  ৎ 🩵 ׂ ⬫ 𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀   ָ࣪  ۰ . ¡! 
⋆ ࣪.𓏲 ๋࣭ Nombre:* @${participant.split`@`[0]}
⋆ ࣪.𓏲 ๋࣭ Enviando el mensaje..
⋆ ࣪.𓏲 ๋࣭ Para desactivar esta función escriba el comando:
⋆ ࣪.𓏲 ๋࣭ #disable antidelete
⋆ ࣪.𓏲 ๋࣭ #enable delete*
┈─┈─┈─┈─┈─┈─┈─┈─┈─┈
`.trim(), msg, {
...
global.dfail = (type, m, conn) => {
    let msg = { 
       ...
       unreg: '¡Hola! y bienvenidx a la base de datos. Para hacer uso del bot, necesitas estar registrado\n📃Para registrarte, necesitas usar el siguiente comando:\n\n.reg nombre.edad\n⋆ ࣪.𓏲 ๋࣭ Un ejemplo: \n.reg Annie.19\n\n⋆ ࣪.𓏲 ๋࣭📌 Espero que hayas seguido correctamente los pasos y completar tu registro para darle uso al bot',
       restrict: '[⚠️ᴬˡᵉʳᵗᵃ ⁱᵐᵖᵒʳᵗᵃⁿᵗᵉ⚠️]\nᴱˢᵗᵉ ᶜᵒᵐᵃⁿᵈᵒ ᴱˢᵗᵃ ᴿᵉˢᵗʳⁱⁿᵍⁱᵈᵒ/ᴰᵉˢᵃᶜᵗⁱᵛᵃ ᴾᵒʳ ᴰᵉˢⁱᶜⁱᵒⁿ ᴰᵉˡ ᴾʳᵒᵖⁱᵉᵗᵃʳⁱᵒ/ᴬ (ᵒʷⁿᵉʳ) ᴰᵉˡ ᴮᵒᵗ'
       }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: '▒̶᩠߲⃡ࠖ᩼᭭݊⃝⃡🩵ࣱꪾ῾֣ؐ  𝘈𝘯𝘯𝘪𝘦𝘉𝘰𝘵 °୭ ', thumbnail: imagen6, sourceUrl: 'https://github.com/4nni33/AnnieBot' }}}}, aa)
